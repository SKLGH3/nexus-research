...
from app.data.store import add_citations, add_task_event, update_task_status
...
class ToolMonitor:
    ...
    def _emit(
        ...
        if thread_id:
            try:
                add_task_event(thread_id, event_type, message, data or {}, payload["timestamp"])
                if event_type == "sources_found":
                    add_citations(thread_id, list((data or {}).get("sources") or []))
                elif event_type == "task_result":
                    update_task_status(thread_id, "completed", result=str((data or {}).get("result") or message))
                elif event_type == "task_cancelled":
                    update_task_status(thread_id, "cancelled")
                elif event_type == "error":
                    update_task_status(thread_id, "failed", error=message)
            except Exception as exc:
                print(f"[Monitor] Persistence failed: {exc}")
        ...
        if self.websocket_manager:
            try:
                ...
                print(f"[Monitor] WebSocket send failed: {exc}")
    ...
    def report_sources(self, sources: list[dict[str, Any]]) -> None:
        """报告并持久化公开网络来源。"""
        self._emit("sources_found", f"发现 {len(sources)} 个公开来源", {"sources": sources})

    def report_task_cancelled(self) -> None:
        """报告任务已被用户取消"""
        self._emit("task_cancelled", "任务已取消")

    def report_error(self, message: str, data: Optional[dict[str, Any]] = None) -> None:
        """报告可展示的任务错误。"""
