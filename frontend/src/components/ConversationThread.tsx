...
import type { Citation, MonitorMessage, OutputFile } from "../types";
...
const TASK_EXAMPLES = [
  ...
  },
  {
    ...
    title: "市场信号分析",
    prompt:
      "请使用数据库查询工具，先发现可用表结构，再分析高分市场信号，按 score 降序给出信号类型、日期、摘要、来源与判断依据。",
        ...
        <div className="artifact-card" key={file.path}>
          ...
          <Tooltip title="下载">
            <Button
              ...
              onClick={() => void downloadSessionFile(threadId, file.path)}
              shape="circle"
            />
          </Tooltip>
        </div>
...
function AssistantMessage({
  ...
  timestamp,
  threadId,
}: Pick<ChatTurn, "events" | "files" | "citations" | "isRunning" | "result" | "timestamp"> & { threadId: string }) {
      ...
      <div className="message-bubble">
        ...
        {citations.length > 0 ? (
          <section className="citation-panel">
            <div className="citation-heading"><span>证据来源</span><strong>{citations.length}</strong></div>
            <ol>{citations.map((source) => <li key={`${source.id}-${source.url}`}><a href={source.url} target="_blank" rel="noreferrer">{source.title}</a><small>{source.domain || source.url.replace(/^https?:\/\//, "").split("/")[0]}</small><p>{source.snippet}</p></li>)}</ol>
          </section>
        ) : null}
